...
pub use engine::{RuleEngine, ExecutionResult};
...
mod tests {
    ...
    fn test_fact_manipulation() {
        let mut fact = Fact::from_object(
            "test".to_string(),
            HashMap::from([
                ("field1".to_string(), FactValue::String("value1".to_string())),
                ("field2".to_string(), FactValue::Number(42.0)),
            ])
        );

        assert_eq!(fact.get_field("field1"), Some(&FactValue::String("value1".to_string())));
        assert_eq!(fact.get_field("field2"), Some(&FactValue::Number(42.0)));

        fact.set_field("field3".to_string(), FactValue::Boolean(true)).unwrap();
    ...
    fn test_grl_parser_complex_rule() {
        ...
        let rule_age = Rule::new(
            ...
            vec![
                ...
                Expression::Assignment(
                    "message".to_string(),
                    Box::new(Expression::String("Customer is eligible for premium service".to_string()))
                )
            ]
        );
        ...
        match &rule.when_condition {
            Expression::Equal(left, right) => {
                match (&**left, &**right) {
                    (Expression::FieldAccess(obj, field), Expression::Boolean(val)) => {
                        match &**obj {
        ...
        match &rule_age.when_condition {
            Expression::GreaterEqual(left_field, left_val) => {
                match (&**left_field, &**left_val) {
                    (Expression::FieldAccess(obj, field), Expression::Number(val)) => {
                        match &**obj {
                            Expression::Variable(obj_name) => {
                                assert_eq!(obj_name, "customer");
                                assert_eq!(field, "age");
        ...
        match &rule_age.then_actions[1] {
            Expression::Assignment(var, val) => {
                assert_eq!(var, "message");
                match &**val {
                    Expression::String(s) => assert_eq!(s, "Customer is eligible for premium service"),
                    _ => panic!("Expected String expression for message value"),
                }
            },
